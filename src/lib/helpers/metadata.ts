...
export const uploadFileToIPFS = async (file: File) => {
    ...
    const auth = "Basic " + Buffer.from(process.env.NEXT_PUBLIC_INFURA_IPFS_API_KEY + ":" + process.env.NEXT_PUBLIC_INFURA_IPFS_API_SECRET).toString("base64");
...
export const getTokenBalance = async (tokenAddress: string) => {
    ...
    const tokenAccount = await getAssociatedTokenAddress(new PublicKey(tokenAddress), Signer.publicKey);

    let tokenAccountBalance = await connection
