...
import { CheckBadgeIcon, ExclamationTriangleIcon, XMarkIcon, LinkIcon } from "@heroicons/react/24/solid";
...
const ContractProcess: React.FC<ContractProcessProps> = ({}) => {
    ...
    const successLabel = useAppSelector((state) => state.callLoader.success.label);
    const successDescription = useAppSelector((state) => state.callLoader.success.description);
    const successLink = useAppSelector((state) => state.callLoader.success.link);
    const successRedirect = useAppSelector((state) => state.callLoader.success.redirect);

    // failure
    const failureLabel = useAppSelector((state) => state.callLoader.failure.label);
    const failureDescription = useAppSelector((state) => state.callLoader.failure.description);
    const failureLink = useAppSelector((state) => state.callLoader.failure.link);
    const failureRedirect = useAppSelector((state) => state.callLoader.failure.redirect);
    ...
    if (callStatus !== "none") {
        return (
            <div
                ...
                {callStatus === "success" && (
                    <div
                        className="relative flex w-[28rem] flex-col items-center justify-between gap-5 rounded-xl bg-light-gray p-6 shadow-xl xl:w-[31rem] xl:p-8 3xl:w-[34rem] 3xl:p-10"
                        onClick={(e) => {
                            e.stopPropagation();
                        }}>
                        <div className="flex flex-col items-center justify-center gap-5">
                            <CheckBadgeIcon className="h-12 w-12 text-new-blue xl:h-14 xl:w-14 3xl:h-16 3xl:w-16" />
                            <div className="text-center text-sm xl:text-base 3xl:text-lg">{successLabel}</div>
                        </div>
                        <XMarkIcon
                            onClick={() => {
                                dispatch(resetLoader());
                        ...
                        <div className="text-xs text-gray-400 xl:text-sm 3xl:text-base">
                            {successDescription}
                            {successLink !== "" && (
                                <AnchorLink className="inline text-white" href={successLink} target="_blank">
                                    <LinkIcon className="ml-5 inline h-3.5 w-3.5" />
                                </AnchorLink>
                            )}
                        </div>
                ...
                {/* failure */}
                {callStatus === "failure" && (
                    <div className="relative flex w-[28rem] flex-col items-center justify-between gap-5 rounded-xl bg-light-gray p-6 shadow-xl xl:w-[30rem] xl:p-8 3xl:w-[34rem] 3xl:p-10">
                        <div className="flex flex-col items-center justify-center gap-5">
                            <ExclamationTriangleIcon className="h-12 w-12 text-red-600 xl:h-14 xl:w-14 3xl:h-16 3xl:w-16" />
                            <div className="text-center text-sm xl:text-base 3xl:text-lg">{failureLabel}</div>
                        </div>
                        <XMarkIcon
                            onClick={() => {
                                dispatch(resetLoader());
                        ...
                        <div className=" text-center text-xs text-gray-400 xl:text-sm 3xl:text-base">
                            {failureDescription}
                            {failureLink !== "" && (
                                <AnchorLink className="inline text-white" href={failureLink} target="_blank">
                                    <LinkIcon className="ml-5 inline h-3.5 w-3.5" />
                                </AnchorLink>
                            )}
                        </div>
