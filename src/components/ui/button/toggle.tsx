...
const ToggleBtn: React.FC<ToggleBtnProps> = ({ option1, option2, label, className, stateChoosen, setStateChoosen }) => {
    const [status, setStatus] = useState(option1);
    return (
        <div className="flex flex-row items-center justify-start">
            {label && <div className="mr-2 text-xs xl:text-sm 3xl:text-base">{label}:</div>}
            <RadioGroup
                value={stateChoosen !== undefined && setStateChoosen !== undefined ? stateChoosen : status}
                onChange={stateChoosen !== undefined && setStateChoosen !== undefined ? setStateChoosen : setStatus}
                ...
                <RadioGroup.Option value={option1}>
                    {({ checked }) => (
                        <span
                            ...
                            {checked && <motion.span className="absolute bottom-0 left-0 right-0 h-full w-full rounded-lg bg-primary shadow-large" layoutId="statusIndicator" />}
