...
const colorThemes: Record<ColorNames, string> = {
    PrimaryOutline: "border-primary text-primary bg-black",
    GreenOutline: "border-new-green text-new-green bg-black",
    RedOutline: "border-new-red text-new-red bg-black",
    GrayOutline: "border-gray-600 text-gray-600 bg-black",
    YellowOutline: "border-yellow-500 text-yellow-500 bg-black",
    WhiteOutline: "border-white text-white bg-black",
    PrimarySolid: "border-black text-black bg-primary",
    GreenSolid: "border-black text-black bg-new-green",
    RedSolid: "border-black text-black bg-new-red",
    GraySolid: "border-black text-black bg-gray-600",
    YellowSolid: "border-black text-black bg-yellow-500",
    WhiteSolid: "border-black text-black bg-white",
};
...
const ButtonNew = forwardRef<HTMLButtonElement, ButtonNewProps>(
    (
        { children, className, isLoading, disabled, fullWidth, shape = "pill", color = "PrimaryOutline", size = "medium", loaderSize = "small", loaderVariant = "scaleUp", onClick, ...buttonProps },
        ...
        return (
            <button
                ...
                className={cn(
                    ...
                    !disabled ? "cursor-pointer hover:-translate-y-0.5 hover:shadow-md focus:-translate-y-0.5 focus:shadow-md" : "cursor-not-allowed",
                    colorClassNames,
                ...
                disabled={disabled}
                {...buttonProps}>
                <span className={cn("flex items-center justify-center gap-2", isLoading && "invisible opacity-0")}>{children}</span>

                {isLoading && <ButtonLoader size={loaderSize} variant={loaderVariant} />}

                {dripShow && (
                    <ButtonDrip x={dripX} y={dripY} color={["white", "gray"].indexOf(color) !== -1 ? "rgba(0, 0, 0, 0.1)" : buttonDripColor} fullWidth={fullWidth} onCompleted={dripCompletedHandle} />
                )}
            </button>
        );
    }
